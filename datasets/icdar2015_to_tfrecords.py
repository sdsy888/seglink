...
if __name__ == "__main__":
    root_dir = util.io.get_absolute_path('~/Dataset/ICDAR2015/TextLocalization/')
    output_dir = util.io.get_absolute_path('~/Dataset/ICDAR2015/TextLocalization/')
    util.io.mkdir(output_dir);

    # for train data
    # training_data_dir = util.io.join_path(root_dir, 'train_image')
    # training_gt_dir = util.io.join_path(root_dir,'train_gt')
    # cvt_to_tfrecords(output_path = util.io.join_path(output_dir, 'icdar2015_train_tfrecord'), data_path = training_data_dir, gt_path = training_gt_dir)

    # for test data
    test_data_dir = util.io.join_path(root_dir, 'test_image')
    test_gt_dir = util.io.join_path(root_dir,'test_gt')
    cvt_to_tfrecords(output_path = util.io.join_path(output_dir, 'icdar2015_test_tfrecord'), data_path = test_data_dir, gt_path = test_gt_dir)
